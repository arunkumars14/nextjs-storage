...
const MobileNavigation = ({ fullName, avatar, email, accountId, $id: ownerId }: Props) => {
                ...
                <SheetContent className='shad-sheet h-screen px-3'>
                    ...
                    <nav className="mobile-nav">
                        <ul className="mobile-nav-list">
                            {navItems.map(({ url, name, icon }: Record<string, string>) => (
                                <Link href={url} key={name} className='w-full' onClick={() => setOpen(false)}>
